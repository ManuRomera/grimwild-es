export class GrimwildChatMessage extends ChatMessage {
	...
	async _updateSpark(event, target) {
		...
		if (!actor) {
			ui.notifications.warn(game.i18n.localize("GRIMWILD.Notification.NoCharacterForSpark"));
			return;
		}
		...
		if (needsUpdate) {
			...
			ui.notifications.warn(game.i18n.format("GRIMWILD.Notification.MaxSpark", { name: actor.name }));
		}
	}
	...
	async _applyHarm(event, target) {
		const actor = game.user.character ?? canvas.tokens.controlled?.[0]?.actor;
		if (!actor) {
			ui.notifications.warn(game.i18n.localize("GRIMWILD.Notification.NoCharacterForMark"));
			return;
		}
